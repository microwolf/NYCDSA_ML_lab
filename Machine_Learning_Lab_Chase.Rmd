...
```{r}
# 3.1 profit lost to returns
# merge by returns 
both = left_join(orders, returns, by = "Order.ID")
just_return = right_join(orders, returns, by = "Order.ID")
sort(both)
returned = filter(both, Returned == "Yes")

returned %>% group_by(year(Order.Date)) %>% summarise(sum(Profit)) 
# only add positive profits?

# 3.2 customer returns
k = returned %>%  group_by(Customer.ID, Customer.Name) %>% summarise(total = n()) 

k %>% filter(total > 1)
k %>% filter(total > 5) %>% arrange(desc(total))

# 3.3 most return region
m = returned %>%  group_by(Region.x) %>% summarise(total = n())
m %>% arrange(desc(total))

# 3.4 sub category
n = returned %>%  group_by(Category) %>% summarise(total = n())
n %>% arrange(desc(total))

i = returned %>%  group_by(Sub.Category) %>% summarise(total = n())
i %>% arrange(desc(total))
```
